...
任意の $\bf{c}∈\mathbb R^{\mathit m×\rm 1}$ に対して、$\bf K_{\mathit {ij}}$を$\bf M$の要素で表現すると、$\bf z_\mathit k = \left[\begin{matrix} c_1 \bf M_{\mathit \rm{1}k}}  \\ \vdots \\ a \end{matrix}\right]$を用いてＡＡＡと書くことができる。

これは、ＰＤＳカーネルが積の下で閉じていることを示している。
