...
* 前章では、線形分類のためのアルゴリズムとしてSVMを紹介
  ...
  * SVM応用上効果的であり、理論的にも正当性が高いという利点がある
...
関数 $K$  は $\mathbb{H}$ 上のカーネルと呼ばれる

$$K \colon  \mathcal{X} \times \mathcal{X} \rightarrow \mathbb{R}$$

* アイデアは、任意の2点 $\forall x, x' \in \mathcal{X}$ に対して、<br>
$K(x, x′)$ がベクトル $\Phi(x)$ と $\Phi(x')$ の内積に等しくなるようなカーネル $K$ を定義すること<br>
(内積を入力空間のものと区別するために，一般的に内積を $<,>$ と表記する)

$$
\forall x, x' \in \mathcal{X}, K(x,x') = <\Phi(x),\Phi(x')> \tag{6.1}
$$
  
特徴量空間と呼ばれるヒルベルト空間 $\mathbb{H}$ への いくつかの写像 $\Phi \colon \mathcal{X} \rightarrow \mathbb{H}$
...
ガウシアンカーネルルは、応用で最も頻繁に使用されるカーネルの一つ
...
定義により、$K$ がPDSであれば、$\bf K$ はすべての $x, x′∈X$ に対してSPSDである

特に$\bf K$ の固有値の積 $det(\bf K\rm)$ は非負でなければならない
...
したがって、$\langle -,-\rangle$ は $\mathbb H_0$ 上のPDSカーネルである
...
さらに、$\langle -,-\rangle$ の再現性を観察する<br>
...
$\mathbb H_0$ は標準的な構成に従って、密なヒルベルト空間 $\mathbb H$ を形成するために完備 () でありうる

また、Cauchy-Schwarzの不等式により、任意の $x ∈\mathcal X$ に対して、$f → \langle f,Φ(x)\rangle$はリプシッツであり、したがって連続である
...
* $\forall x,x'∈\mathcal X$ に対して $K(x,x')$ を持つ $Φ:\mathcal X \rightarrow \mathbb H$ が存在するような任意のヒルベルト空間 $\mathbb H$は、$K$ に関連付けられた特徴空間 (feature space)と呼ばれ、$Φ$ は特徴写像 (feature mapping) と呼ばれる
...
任意のカーネルKに対して、AAAで定義された正規化カーネルK′を関連付けることができる。

定義により，正規化カーネルK′に対して，K (x, x) != 0となるようなすべてのx ∈Xに対して，K′(x, x) = 1.

正規化カーネルの例は、パラメータσ＞０のガウスカーネルであり、これは、Ｋ′に関連付けられた正規化カーネルである：(x,x′)→exp(x-x′)
