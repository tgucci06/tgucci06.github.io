...
この計量は類似度を定義するために使用できる
  ...
  * (例) ガウシアン・カーネルは $\rm exp\left(-\mathit {d}^{\,2}\right)$ の形をしている<br>
  ここで $d$ は入力ベクトル空間の計量である

以下のようないくつかの自然な疑問が生じる<br>

  * ヒルベルト空間のメトリックから他にどのような PDS カーネルを構築できるか？<br>
  * $\rm exp\left(-\mathit {d}^{\,2}\right)$ がPDSであることを保証するために、$d$ はどのような技術的条件を満たすべきか？

これらの疑問に対処するのに役立つ自然な数学的定義が負定値対称 (NDS) カーネルの定義である

### Definition 6.14 (Negative definite symmetric (NDS) kernels )

カーネル $K : \mathcal X × \mathcal X → \mathbb R$ が対称であり、$\{x_1,...,x_m\} \subseteq \mathcal X$ と  $1^\top \bf c \rm = 0$ となる $\bf{c} ∈\mathbb R^{\rm m×1}$ において以下のような場合、そのカーネルは負定値対称 (NDS) であると言われる
...
確かに $\bf c ∈ \mathbb R^{\rm m×1}$ で $\sum^m_{i = 1}c_i = 0$ とする 
...
$$
\begin{aligned}
\sum^m_{i,j = 1} c_ic_j\|\bf x_{\mathit i} -x_{\mathit j}\|^\rm 2 
&= \sum^{\mathit m}_\mathit{i,j \rm = 1}c_ic_j\left(\bf x_{\mathit i} - x_{\mathit j}\right)\cdot\left(\bf x_{\mathit i} - x_{\mathit j}\right)\\
&= \sum^{\mathit m}_\mathit{i,j \rm = 1}\mathit c_ic_j\left(\|\bf x_{\mathit i}\|^{\rm 2} + \|\bf x_{\mathit j}\|^{\rm 2} -\rm 2\bf x_{\mathit i} \cdot x_{\mathit j}\right)\\
&= \sum^{\mathit m}_\mathit{i,j \rm = 1}\mathit c_ic_j \left(\|\bf x_{\mathit i}\|^{\rm 2} + \|\bf x_{\mathit j}\|^{\rm 2}\right) - 2\sum^m_{i = 1}c_i\bf x_{\mathit i}\cdot\sum^{\mathit m}_{\mathit j \rm = 1}c_\mathit j\bf x_{\mathit j}\\
&=  \sum^{\mathit m}_\mathit{i,j \rm = 1}\mathit c_ic_j \left(\|\bf x_{\mathit i}\|^{\rm 2} + \|\bf x_{\mathit j}\|^{\rm 2}\right) - 2\left\|\sum^m_{i=1}c_i\bf x_{\mathit i}\right\|^2 \\
&\le \sum^m_{i,j = 1}c_ic_j \left(\|\bf x_{\mathit i}\|^{\rm 2} + \|\bf x_{\mathit j}\|^{\rm 2}\right)\\
...
$K'$ を $\forall x_0$ について $\forall x,x' \in \mathcal X,\;K'(x, x') = K(x, x_0) + K(x', x_0) - K(x, x') - K(x_0, x_0)$ によって定義されるとすると、$K'$ がPDSであれば、$K$ はNDSである

**Proof**

$K'$ がPDSであるとし、$\forall x_0$ について、$K(x, x') = K(x, x_0)+K(x_0, x')-K(x_0, x_0)-K'(x, x')$ を持つような $K$ を定義する

そして、$\bf c^\top1 \rm= 0$ となるような $\forall \bf c\in\mathbb R^{\mathit m}$ と、任意の点の集合 $(x_1,...,x_m)∈\mathcal X^m$ において、以下のように $K$ がNDSであることを証明する
...
$K: \mathcal X × \mathcal X → \mathbb R$ を、 $\forall x,x′ ∈\mathcal X$ において、$x = x'$ ならば、$K(x,x') = 0$ となるようなNDSカーネルとする

すると、$x,x' ∈ \mathcal X$ において、以下のようなヒルベルト空間 $\mathbb H$ と写像 $Φ:\mathcal X→ \mathbb H$ が存在する
...
したがって、定理の仮説の下では、$\sqrt{K}$ はメトリックを定義する

この定理は、$\mathbb R$ のカーネル $(x,x') \mapsto \exp(-|x - x'|^p)$ が $p > 2$ に対してPDSではないことを示すために用いることができる

そうでなければ、$\forall t > 0, \{x_1,...,x_m\} ⊆\mathcal X, \bf c ∈ \mathbb R^{\mathit m×\rm 1}$ に対して、次のようになる

$$
\sum^{m}_{i,j = 1}c_ic_je^{-t|x_i-x_j|^p} = \sum^{m}_{i,j = 1}c_ic_je^{-|t^{1/p}x_i-t^{1/p}x_j|^p} \ge 0
$$

これは、$(x, x') \mapsto |x - x'|^p$ が $p > 2$ に対して NDS であることを意味する

## Sequence kernels

一般的に使用される多項式カーネルやガウシアンカーネルを含む、前節で与えられた例は、すべてベクトル空間上のPDSカーネルのためのものである

実際に見られる多くの学習タスクでは、入力空間Xはベクトル空間ではない

  * 実際に分類する例としては、タンパク質配列、画像、グラフ、解析木、有限オートマトン、またはベクトルとして直接与えられない他の離散構造が考えられる

PDS カーネルは、もともとベクトル空間のために設計された SVM のようなアルゴリズムを、このようなオブジェクトの分類に拡張するための方法を提供する

しかし、これらの構造に対してどのようにPDSカーネルを定義すればよいか？

このセクションでは、シーケンスカーネル、すなわち、シーケンスや文字列のためのカーネルの具体的なケースに焦点を当てる

PDS カーネルは、他の離散構造に対しても同様の方法で定義することができる

シーケンスカーネルは、計算生物学や自然言語処理に適用される学習アルゴリズムに特に関連しており、これらはどちらも重要な応用である

どのようにしてシーケンスの類似度であるシーケンスのPDSカーネルを定義することができるのか？

一つの考えは、二つの配列、例えば二つの文書や二つの生物配列を、共通の部分文字列や部分配列を共有している場合に、類似していると宣言すること

  * (例) 二つの配列間のカーネルは、それらの共通の部分文字列のカウントの積の和で定義される

しかし、どの部分文字列がその定義に使われるべきか？

ほとんどの場合、一致する部分文字列の定義に flexibility (柔軟性) が必要になるだろう

  * (例) 計算生物学の応用では、一致するものは不完全になる可能性がある

そのため、いくつかの不一致、場合によってはギャップ、ワイルドカードを考慮する必要があるかもしれない

より一般的には、様々な置換を許容する必要があるかもしれないし、共通の部分文字列に異なる重みを割り当てて、マッチする部分文字列を強調し、他の部分文字列を強調しないようにしたいと思うかもしれない

この議論から分かるように、多くの異なる可能性があり、そのようなカーネルを定義するための一般的なフレームワークが必要

以下では、この議論で考えられているすべてのカーネルを含む、**シーケンスカーネル**、**有理カーネル** のための一般的なフレームワークを紹介

また、それらの計算のための一般的で効率的なアルゴリズムを記述し、いくつかの例を用いてそれらを説明

これらのカーネルの定義は、Weighted transducers の定義に依存している

したがって、これらの定義といくつかの関連するアルゴリズムから始める


### 6.5.1 Weighted transducers

シーケンスカーネルは、Weighted transducers を用いて効果的に表現し、計算することができる

以下の定義において、$Σ$ は有限の入力アルファベット、$∆$ は有限の出力アルファベット、$\epsilon$ は空の文字列または null label を表し、それらは任意の文字列と連結しても変更されない

### Definition 6.19

Weighted transducer $T$ は、$\mathit 7$ -タプル (組) $T = (Σ, ∆, Q, I, F, E, \rho)$ 
...
図6.4にweighted finite-state transducer (重み付き有限状態 transducer) の例を示す
...
パス $π$ の入力ラベルは、$π$ に沿って入力ラベルを連結して得られる$Σ^∗$ の文字列要素である
...
weighted transducer $T$ が文字列のペア $(x, y) ∈Σ^∗ × ∆^∗$ に関連付ける重みを $T(x,y)$ と表し、入力ラベル $x$ と出力ラベル $y$ を持つすべての受け入れ可能なパスの重みを合計して得られる

例えば、図6.4の transducer では、入力ラベル $aab$ と出力ラベル $baa$ を持ち、重みが $3×1×4×2$ のパスと、重みが $3×2×3×2$ のパスをもつので、$(aab,baa)$ のペアに，3×1×4×2+3×2×3×2の重みを与えている

非循環transducer、つまり循環しないtransducer $T$ のすべての accepting paths の重みの合計は、一般的な最短距離アルゴリズムまたは forward- backward アルゴリズム (近接勾配法)を用いて、線形時間、つまり $O\left(|T|\right)$ で計算できる

  * これらは単純なアルゴリズムであるが、詳細な説明をすると本章の主旨から逸脱してしまう

### Composition

weighted transducers の重要な操作に合成がある<br>
これは、2つ以上の weighted transducers を組み合わせて、より複雑な weighted transducers を形成するために使用できる

これから説明するように、この操作はシーケンスカーネルの作成と計算に役立つ

その定義は、関係の合成の定義に従う

2つの weighted transducers $T_1 = (Σ, ∆, Q_1, I_1, F_1, E_1, ρ_1)$ と $T_2 = (Δ, Ω, Q_2, I_2, F_2, E_2, ρ_2)$ が与えられたとき、$T_1$ と $T_2$ を合成した結果は、$T_1 \circ T_2$で示される weighted transducers であり、 $\forall x∈Σ^∗$ と  $\forall y ∈ Ω^∗$ に対して以下の(6.19)式で定義され、総和はアルファベット $Δ$ 上のすべての文字列 $z$ に対して実行される

$$
\left(T_1 \circ T_2\right)(x,y) = \sum_{z \in \Delta^*}T_1(x,z) \cdot T_2(z,y) \tag{6.19}
$$

2つの重み付き変換器の合成を計算する一般的で効率的なアルゴリズムが存在する

$T_1$ の入力側または $T_2$ の出力側に $\epsilon$ がない場合、$T_1 \circ T_2 = (Σ,Δ,Q,I,F,E,ρ)$ の状態は、$T_1$ の状態と $T_2$ の状態からなるペア、$Q ⊆ Q_1×Q_2$ で識別できる

初期状態とは、元の transducers の初期状態をペアにして得られるもので、$I = I_1 × I_2$ であり、同様に最終状態は $F = Q∩ (F_1 × F_2)$ で定義される

ある状態 $(q_1, q_2) ∈ F_1 × F_2$ における最終重みは $ρ(q)=ρ_1(q_1)ρ_2(q_2)$ であり、$q_1$ と $q_2$ の最終重みの積となる

遷移は、$T_1$ と$T_2$ の適切な遷移から、$T_1$ の遷移と $T_2$ の遷移をマッチングさせて得られる
...
ここで、$\uplus$ は、$\{1, 2\} \uplus \{1, 3\} = \{1, 1, 2, 3\}$ のように、多集合の標準的な結合操作を表し、遷移の重複度 (multiplicity) を保持する

最悪の場合、状態 $q_1$ を去る $T_1$ のすべての遷移は、状態 $q'_1$を去る $T_2$ のすべての遷移と一致するので、合成の空間と時間の複雑さは2次関数的になる：$O\left(|T1||T2|\right)$

実際には、このようなケースは稀であり、合成は非常に効率的である

図6.5は、あるケースでのアルゴリズムを示している

図6.6に示すように、$T_1$ が出力 $\epsilon$ ラベルを認め、$T_2$ が入力 $\epsilon$ ラベルを認めている場合、先ほどのアルゴリズムでは、冗長な $\epsilon$ パスを作成してしまい、誤った結果になる可能性がある

元の transducers のマッチするパスの重みは $p$ 回数えられることになる<br>
ここで$p$は合成結果の冗長なパスの数である

この問題を回避するためには、composite transducer (複合変換器) から1つの $\epsilon$ -パスを除いてすべての $\epsilon$ -パスをフィルタリングしなければならない

図6.6では、その経路の選択肢の一つを太字で示していますが、この場合は最短の経路である

驚くべきことに、このフィルタリング自体が有限状態変換器 $F$ としてエンコードできる（図6.6b）

このフィルタを適用するためには，まず $T_1$ と $T_2$ を $\epsilon$ のセマンティクスを明示する補助記号で補強する必要がある

図6.6に示すように、$\tilde{T_1} \left(\tilde{T_2}\right)$ を $T_1 \;(T_2)$ から出力（入力）$\epsilon$ ラベルを$\epsilon_2 \;(\epsilon_1)$ に置き換えることで得られる weighted transducer とする

したがって、シンボル $\epsilon_1$ とのマッチングは、$T_1$ の同じ状態にとどまり、入力 $\epsilon$ で $T_2$ の遷移を受けることに対応する

$\epsilon_2$ は対称的な方法で記述することができる

filter transducer $F$ は、$(\epsilon_1,\epsilon_1)$ の直後にマッチング $(\epsilon_2,\epsilon_2)$ を行うことを許可しない

対称性により、$(\epsilon_2, \epsilon_2)$ の直後のマッチング $(\epsilon1, \epsilon1)$ も許可されない

同様に、マッチング $(\epsilon_1,\epsilon_1)$ の直後に $(\epsilon_2,\epsilon_1)$ が続く場合も、マッチング $(\epsilon_2,\epsilon_1)(\epsilon_1,\epsilon_1)$ を経由する経路が可能なので、フィルタ $F$ では許可されない

同様に、$(\epsilon_2, \epsilon_2)(\epsilon_2, \epsilon_1)$ も除外される

filter transducer $F$ は、正確には言語 $L$ の補数を受け入れるペア上の有限オートマトンであることを検証するのは難しくない
ここで、$σ =\left\{(\epsilon_1, \epsilon_1), (\epsilon_2, \epsilon_2), (\epsilon_2, \epsilon_1), x\right\}$ である
...
このように、フィルタ $F$ は、各 $\epsilon$ 配列の合成において、正確に1つの $\epsilon$ 経路が許容されることを保証する

合成の正しい結果を得るためには、すでに説明した$\epsilon$ -free 合成アルゴリズムを使用して、次のように計算すればよい

$$
\tilde{T_1} \circ F \circ \tilde{T_2} \tag{6.20}
$$

filter transducer $F$ のサイズは一定なので、一般的な合成の複雑さは $\epsilon$ -free合成と同じで、$O(|T_1||T_2|)$ となる
...
カーネル $K : Σ^∗ × Σ^∗ → \mathbb R$ は、ある weighted transducer $U: \forall x,y ∈ Σ^∗,K(x,y) = U(x,y)$ によって定義される写像と一致する場合、有理であるという

transducer $\forall T:\Sigma^* \times \Delta^* \rightarrow \mathbb R$ について，$T$ の逆写像を $T^{-1} : \Delta^* \times \Sigma^* \rightarrow \mathbb R$ とすると，これは $T$ から各遷移の入力ラベルと出力ラベルを入れ替えて得られる transducer である
...
任意の weighted transducer $T = \left(Σ,∆,Q,I,F,E,ρ\right)$ において、関数 $K = T ◦ T^{-1}$ はPDS rational kernelである
...
Kは次のように定義されるカーネル列 $(K_n)_{n \ge 0}$の点極限である
...
これは、$\bf K_n$が、$\bf{A} = \rm(\mathit{K_n(x_i,z_j}))_{\mathit i∈[1,\mathit m],\mathit j∈[1,\mathit N]}$ として、$\bf K_n = AA^\top$ と書けることからもわかる<br>
ここで、$z_1,...,z_N$ は、長さが最大 $n$ の$Σ^∗$ の文字列の集合の任意の列挙である

このように、$K$ はPDSカーネル $\left(K_n\right)_{n∈\mathbb N}$ の列の点極限としてPDSである (証明終了)
