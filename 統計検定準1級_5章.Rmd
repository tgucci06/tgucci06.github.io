...
平均や分散はできれば暗記しておきたいが、確率関数が与えられていれば確率母関数から導出は可能  
...
壺の中に $N$ 個の玉が入っている  
($N$ 個中 $M$ 個は赤玉、$N - M$ 個は白玉)  
壺の中の玉をよくかき混ぜ、壺から $n$ 個の玉を取り出す

$n$ 個の玉のうち、赤玉の個数を $Y$ とする  
$n = 1$ ならば、$Y$ は 成功確率 $\frac{M}{N}$ のベルヌーイ分布 $Bin\left(1, \frac{M}{N}\right)$ に従う

復元無作為抽出 (1つ取り出すごとにその玉を壺に戻し、次の玉を取り出す) のときは  
...
ここで、$_r \mathrm H_y$ は「1回目の成功より前」、「1回目と2回目の成功の間」、...、「$r-1$ 回目の成功より後」という  
$r$ 個の中から重複を許して、(失敗が起こる期間として) $y$ 個選ぶ重複組合せの総数  
(各期間に起こる失敗の回数としての非負整数 $x_1,\dots,x_r\:\:(x_1 + \cdots x_r = y)$ の決め方の総数) 

## 重複組合せの総数と負の二項係数

(補足) 重複組合せの総数 $_r \mathrm H_y$ を **負の二項係数** を用いて表す

負も含めた任意の整数 (実数、複素数などと考えて良い) $a$ と非負整数 $b$ に対し、一般化された二項係数 $\binom{a}{b}$ は
...
で定義される  
($b = 0$ のときは $\binom{a}{0} = 1/0! = 1$)

これを用いると、

$$
_r \mathrm H_y = {}_{y + r -1} C_y = \frac{(y + r -1)(y + r -2) \cdots (r + 1)r}{y!} = (-1)^y \cdot \frac{(-r)(-r -1) \cdots (-r -y + 1)}{y!} = (-1)^y \binom{-r}{y}
$$

となる

このように負の二項分布の確率関数 (5.13) の係数に現れる $_r \mathrm H_y$ は、  
負の $-r$ に対する二項係数 $\binom{-r}{y} = (-r)(-r -1) \cdots (-r -y + 1)/y!$ を用いて表すことができる
...
他方、 $Y \sim NB(r,p)$ の確率母関数は、負の二項係数を用いて以下のように書ける
...
確率母関数 $G(s) = \left(\frac{p}{1 - qs}\right)^r$ の形より、$Y_1 \sim NB(r_1,p)$ と $Y_2 \sim NB(r_2,p)$ が独立ならば、<br>
...
$K \:(\ge 2)$ 個の結果 $1,\dots,K$ のいずれか1つが起こる試行を考える  
